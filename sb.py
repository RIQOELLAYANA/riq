...
parser = argparse.ArgumentParser(description='Sadbot By Riq')
...
line = LINE("arumaini86@gmail.com","arumhurur1304") #Input Your Email and Password Here
#=======================================================================================================================
...
def sendFooter(to, isi):
    data = {
        ...
        "sentBy": {
            ...
            "linkUrl": "https://line.me/ti/p/~5iveminutes" # DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
        }
    }
...
def debug():
	...
	return "「 Speed 」\nType: Speed\n • Took : %.0fms\n • Taken: %.1f" % (get_profile_took,get_profile_time)
def restoreProfile():
    profile = line.getProfile()
...
def executeCmd(msg, text, txt, cmd, msg_id, receiver, sender, to, setKey):
    ...
    if to in silent['silent']:
        return
    if cmd == 'logout':
        sendFooter(to, 'Sadbot will logged out')
...
#================BATAS================
        ...
        line.sendMessageMusic(to, title=line.getContact(myMid).displayName, subText=str(line.getContact(myMid).statusMessage), url='https://line.me/ti/p/~5iveminutes', iconurl="http://dl.profile.line-cdn.net/{}".format(line.getContact(myMid).pictureStatus), contentMetadata={})
    elif cmd == 'runtime':
                ...
                try:
                    ...
                    pass 
						...
						if len(ban["blacklist"]) > 0:
							...
							for aa in range(k+1):
								...
								for a in h[aa*20:(aa+1)*20]:
									no+=1
									if no == len(h):msgas+='\n├➢ {}. @!\n╰───[ Riq Sadbot ]'.format(no)
									else:msgas += '\n├➢ {}. @!'.format(no)
								sendMention(to, msgas, h[aa*20:(aa+1)*20])
						else:
							sendFooter(to,"「 Doesn't Have Any Blacklist User 」")
    elif cmd == 'whitelist':
						if len(settings["whitelist"]) > 0:
							...
							sendFooter(to,"「 Doesn't Have Any whitelist User 」")
...
#================Protection================
						...
						if len(settings["protectjoin"]) > 0:
							...
							sendFooter(to,"「 Doesn't List Lock Join 」")
						...
						if len(settings["protectkick"]) > 0:
							...
							sendFooter(to,"「 Doesn't List Lock Kick 」")
						...
						if len(settings["protectinvite"]) > 0:
							...
							sendFooter(to,"「 Doesn't List Deny Invitation 」")
						...
						if len(settings["protectcancel"]) > 0:
							...
							sendFooter(to,"「 Doesn't List Lock Cancel 」")
						...
						if len(settings["protectqr"]) > 0:
							...
							sendFooter(to,"「 Doesn't List QR Protection 」")					
                                ...
                                sendFooter(to,"• Protectlist •\n\n- QR Protection :\n"+ma+"\n- Lock Kick :\n"+mb+"\n- Lock Join :\n"+md+"\n- Deny Invitation :\n"+me+"\n- Lock Cancel :\n"+mf+"\nTotal「%s」Protect Group\n\nRiq Sadbot" % (str(len(settings["protectqr"])+len(settings["protectkick"])+len(settings["protectjoin"])+len(settings["protectinvite"])+len(settings["protectcancel"]))))
                            ...
                            if spl == 'on':
                                if msg.to in settings["protectinvite"]:
                                     msgs = "Deny Invitation Already Enabled "
                                else:
                                ...
                                sendFooter(to, "「 Deny Invitation 」\n" + msgs)
                                  ...
                                  if msg.to in settings["protectinvite"]:
                                       ...
                                       msgs = "Deny Invitation Already Disabled "
                                  sendFooter(to, "「 Deny Invitation 」\n" + msgs)        
                            ...
                            if spl == "off":
                                ...
                                if msg.to in settings["protectcancel"]:
                                   ...
                                   msgs = "Maximum Protection Already Disabled "
                                sendFooter(to, "「 Maximum Protection 」\n" + msgs)
#================BATAS================
    elif cmd == 'about':
        res = '╭───[ About ]'
    ...
    try:
        ...
        if op.type == 19 or op.type == 133:
          if op.param3 in myMid:
                ban["blacklist"].append(op.param2)
                group = "ucc30848fde8b9c522b5263360a2d4e8e" #Can Change for Mid Group or Line Personal Chat
                ...
                data = {
                                           ...
                                           "iconUrl": "https://obs.line-scdn.net/{}".format(line.getContact(myMid).pictureStatus),
                                           "linkUrl": "https://line.me/ti/p/~5iveminutes",
                                         }
                                     }
            ...
            if msg.contentType == 0:
                ...
                if wait["wblacklist"] == True:
                    if msg.contentMetadata["mid"] in ban["blacklist"]:
                        ...
                        sendFooter(to,"「 Blacklist 」\nSuccess Add Contact To Blacklist ")
                        wait["wblacklist"] = False
                if wait["dblacklist"] == True:
                    if msg.contentMetadata["mid"] in ban["blacklist"]:
                        ban["blacklist"].remove(msg.contentMetadata["mid"])
                        sendFooter(to,"「 Blacklist 」\nSuccess Delete Contact From Blacklist ")
                        wait["dblacklist"] = False
                    else:
                        wait["dblacklist"] = False
                ...
                if wait["wwhitelist"] == True:
                    if msg.contentMetadata["mid"] in settings["whitelist"]:
                        sendFooter(to,"「 Whitelist 」\nContact Already In Whitelist ")
                        wait["wwhitelist"] = False
                    else:
                        settings["whitelist"].append(msg.contentMetadata["mid"])
                        sendFooter(to,"「 Whitelist 」\nSuccess Add Contact To Whitelist ")
                        wait["wwhitelist"] = False
                        ...
                        if Name in cctv['sidermem'][op.param1]:
                            pass
                                ...
                                if " " in Name:
                                    nick = Name.split(' ')
                                    if len(nick) == 2:
                                        line.sendMessageMusic(op.param1, kontak.displayName, 'SAYANG AKU GA ? -Riq', 'line.me/ti/p/~5iveminutes', "https://obs.line-apps.com/os/p/{}".format(str(kontak.mid)))
